...
A qualification cycle is invalid if it is incremental-only, partial, skipped, failed, limitation-affected, ambiguous, stale, reused, or not demonstrably a successful full review of the complete canonical-base-to-head delta. A clean result or `CodeRabbit=success` for only a subset of changes does not qualify.

The request, acknowledgement, processing evidence, terminal cycle record, and authoritative review record must all agree on canonical base SHA/tree, repository, PR, exact head, and cycle ID. Request and acknowledgement bodies must remain hash-identical to their pinned snapshots. The acknowledgement must remain GitHub-authenticated as `coderabbitai[bot]` ID `136622811`. The terminal cycle record and authoritative review record are one immutable terminal review-result identity, not two interchangeable records.
...
Immediately before merge, independently re-read and require unchanged: canonical main SHA/tree, PR base SHA/tree and their equality to canonical main/tree, PR state/head, reviewed tree, reviewed document blob, exact one-path delta, exact-head CI, unique full-review cycle evidence, cycle/request/ack/processing/terminal canonical-base SHA/tree equality, request/ack body hashes and authenticated ack identity, terminal review identity/body hash, terminal-result identity equality, CodeRabbit status bound to the exact head, zero current material threads, zero current material risks, and mergeability.
