...
The `PRODUCT / BENCHMARK / RUNTIME ... INVOCATION = NOT_AUTHORIZED` non-grant applies only to product behavior, benchmark execution, and runtime invocation surfaces. It does not prohibit or narrow the independent external semantic review channels required solely as governance evidence to qualify this authorization candidate or the later five-path reconciliation under the canonical provider-neutral review-quorum amendment.
