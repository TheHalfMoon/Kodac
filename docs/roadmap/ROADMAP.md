...
| **K4** | Ecosystem Compatibility & Capability Registry | **DEFINED / IN PROGRESS — K4-R1 AND K4-R2 CANONICAL FOR THEIR BOUNDED SCOPES** | K4-R1 and K4-R2 only as canonically adopted; K4-R3+ not authorized |
...
The shared `src/index.ts` trigger inherited from the one-time K4-R1 implementation gate was canonically hardened before K4-R2 source began. `docs/planning/KODAC_K4_R2_REVIEW_HARDENING_SCOPE_EXTENSION_2026-08-24.md` records the continuing K4-R1 canonical-regression gate while leaving the K4-R2 five-path implementation allowlist unchanged.
...
4. K4 is defined and K4-R1 and K4-R2 are canonical only for their exact bounded data-only scopes; K4-R3+ remains unauthorized, and no external standard name, declaration, metadata, digest, cache hint, or registry membership is executable authority.
