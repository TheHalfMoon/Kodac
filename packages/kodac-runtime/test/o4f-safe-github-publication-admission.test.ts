...
function fixture(opts: { rows?: Obj[]; support?: string[]; content?: Record<string, Obj>; base?: string; head?: string; repoId?: string; prId?: string; repo?: string; pr?: number; times?: string[] } = {}) {
...
const cases: Array<[string, () => void | Promise<void>]> = [
  ...
  ["time-only O4-B difference preserves logical slot while changing execution and request identities", async () => { const claims = (a: O4dO4cReviewerExecutionAdmissionResult) => [claim(a, { range: { startLine: 1, endLine: 1 } })]; const r1 = admit(await buildBundle({ fixture: { times: ["2026-09-12T00:00:00.000Z", "2026-09-12T00:00:01.000Z"] }, claims })); const r2 = admit(await buildBundle({ fixture: { times: ["2026-09-12T00:10:00.000Z", "2026-09-12T00:10:01.000Z"] }, claims })); assert.notEqual(r1.o4eExecutionIdentity, r2.o4eExecutionIdentity); assert.equal(r1.publicationRequests[0]!.publicationSlotIdentity, r2.publicationRequests[0]!.publicationSlotIdentity); assert.equal(r1.publicationRequests[1]!.publicationSlotIdentity, r2.publicationRequests[1]!.publicationSlotIdentity); assert.notEqual(r1.publicationRequests[0]!.publicationRequestIdentity, r2.publicationRequests[0]!.publicationRequestIdentity); assert.notEqual(r1.publicationAdmissionIdentity, r2.publicationAdmissionIdentity) }],
  ["different claim summaries share logical summary slot but change request identity", async () => { const r1=admit(await buildBundle({claims:(a)=>[claim(a,{summary:"one"})]})); const r2=admit(await buildBundle({claims:(a)=>[claim(a,{summary:"two"})]})); assert.notEqual(r1.o4eExecutionIdentity,r2.o4eExecutionIdentity); assert.equal(r1.publicationRequests[0]!.publicationSlotIdentity,r2.publicationRequests[0]!.publicationSlotIdentity); assert.notEqual(r1.publicationRequests[0]!.publicationRequestIdentity,r2.publicationRequests[0]!.publicationRequestIdentity) }],
  ["changed reviewed head changes logical slot", async () => { const r1 = admit(await buildBundle()); const r2 = admit(await buildBundle({ fixture: { head: ALT_HEAD } })); assert.notEqual(r1.publicationRequests[0]!.publicationSlotIdentity, r2.publicationRequests[0]!.publicationSlotIdentity) }],
  ["changed task changes logical slot", async () => { const r1 = admit(await buildBundle({ taskId: "review-pr-42" })); const r2 = admit(await buildBundle({ taskId: "review-pr-alt" })); assert.notEqual(r1.publicationRequests[0]!.publicationSlotIdentity, r2.publicationRequests[0]!.publicationSlotIdentity) }],
  ["changed policy changes logical slot", async () => { const r1 = admit(await buildBundle({ policy: "policy:v1" })); const r2 = admit(await buildBundle({ policy: "policy:v2" })); assert.notEqual(r1.publicationRequests[0]!.publicationSlotIdentity, r2.publicationRequests[0]!.publicationSlotIdentity) }],
  ["changed inline claim identity changes inline slot", async () => { const r1 = admit(await buildBundle({ claims: (a) => [claim(a, { claimKey: "claim:alpha", summary: "alpha", range: { startLine: 1, endLine: 1 } })] })); const r2 = admit(await buildBundle({ claims: (a) => [claim(a, { claimKey: "claim:beta", summary: "beta", range: { startLine: 1, endLine: 1 } })] })); assert.equal(r1.publicationRequests[0]!.publicationSlotIdentity, r2.publicationRequests[0]!.publicationSlotIdentity); assert.notEqual(r1.publicationRequests[1]!.publicationSlotIdentity, r2.publicationRequests[1]!.publicationSlotIdentity) }],
  ["changed inline path changes inline slot", async () => { const rows = [changed("src/a.ts"), changed("src/b.ts")]; const byPath = (p: string) => (a: O4dO4cReviewerExecutionAdmissionResult) => [claim(a, { path: p, evidenceItemIds: [a.items.find((x) => x.subjectPath === p)!.itemId], range: { startLine: 1, endLine: 1 } })]; const r1 = admit(await buildBundle({ fixture: { rows }, claims: byPath("src/a.ts") })); const r2 = admit(await buildBundle({ fixture: { rows }, claims: byPath("src/b.ts") })); assert.notEqual(r1.publicationRequests[1]!.publicationSlotIdentity, r2.publicationRequests[1]!.publicationSlotIdentity) }],
  ["changed inline line changes inline slot", async () => { const r1 = admit(await buildBundle({ claims: (a) => [claim(a, { range: { startLine: 1, endLine: 1 } })] })); const r2 = admit(await buildBundle({ claims: (a) => [claim(a, { range: { startLine: 2, endLine: 2 } })] })); assert.notEqual(r1.publicationRequests[1]!.publicationSlotIdentity, r2.publicationRequests[1]!.publicationSlotIdentity) }],
