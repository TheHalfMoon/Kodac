...
import {
  P7_R10_WORKSPACE_REFERENCE_EVIDENCE_BINDING_VERSION,
  P7_R10_WORKSPACE_REFERENCE_EVIDENCE_BOUND_STATE,
  buildP7WorkspaceReferenceEvidenceBinding,
  p7WorkspaceReferenceEvidenceBindingIdentity,
...
const SESSION_ID = "session-p7-r10"
...
function claim(): Record<string, unknown> {
  return {
    claimKey: "p7-r10-source-finding",
    review: {
      reviewRunId: "review-run-p7-r10",
      reviewerId: "provider:test",
      reviewerVersion: "v1",
      policyIdentity: "policy:p7-r10-test",
      canonicalBase: BASE,
      reviewedHead: HEAD,
    },
    path: "src/example.ts",
    range: { startLine: 10, endLine: 12 },
    summary: "A confirmed finding eligible for bounded workspace-reference evidence binding.",
...
function sourceProposal(): P7ImmutablePatchProposal {
  const runtime = new ReviewerIntelligenceRuntime({ adjudicatorId: "kodac:p7-r10-test-adjudicator" })
  ...
  return buildP7ImmutablePatchProposal({
    ...
    proposerIdentity: "kodac:p7-r10-test-proposer",
...
function sourceAuthorization(source: P7ImmutablePatchProposal): P7PatchApplicationAuthorization {
  return buildP7PatchApplicationAuthorization({
    ...
    declaration: {
      authorizerIdentity: "kodac:p7-r10-test-authorizer",
...
function verificationReport(plan: P7VerificationPlanInput): MutableRecord {
  ...
  const base = [
    ...
    { id: "git.diff", category: "diff", status: "pass", summary: "Diff evidenced.", evidence: [{ kind: "receipt", ref: APPLY_RECEIPT_ID }] },
    {
      ...
      evidence: [{ kind: "receipt", ref: APPLY_RECEIPT_ID }, { kind: "receipt", ref: TEST_RECEIPT_ID }, { kind: "receipt", ref: TYPES_RECEIPT_ID }],
    },
    {
      id: "evidence.policy",
      category: "policy",
...
function fixtureInput(report?: MutableRecord): P7WorkspaceReferenceEvidenceBindingBuildInput {
...
function reportFrom(input: P7WorkspaceReferenceEvidenceBindingBuildInput): MutableRecord {
  return structuredClone(
    input.sourceAgentCompletionEvidenceBindingInput.sourceCommandSuccessEvidenceBindingInput
      .sourceVerificationReportBindingInput.verificationReport,
  ) as MutableRecord
}

function rebuildWithReport(report: MutableRecord): P7WorkspaceReferenceEvidenceBindingBuildInput {
  return fixtureInput(report)
}

const schema = JSON.parse(
  readFileSync(new URL("../../../schema/p7-workspace-reference-evidence-binding.schema.json", import.meta.url), "utf8"),
) as UnknownRecord
const sourceText = readFileSync(
  new URL("../src/remediation/p7-workspace-reference-evidence-binding.ts", import.meta.url),
  "utf8",
)

test("P7-R10 builds and validates one deterministic WORKSPACE_REFERENCE_EVIDENCE_BOUND record", () => {
  const input = fixtureInput()
  const built = buildP7WorkspaceReferenceEvidenceBinding(input)
  assert.equal(built.version, P7_R10_WORKSPACE_REFERENCE_EVIDENCE_BINDING_VERSION)
  assert.equal(built.state, P7_R10_WORKSPACE_REFERENCE_EVIDENCE_BOUND_STATE)
  assert.equal(built.workspaceReference, WORKSPACE)
  assert.equal(built.workspaceReferenceDigest, sha256(WORKSPACE))
  assert.deepEqual(built.workspaceCheckEvidence, [{ kind: "workspace", ref: WORKSPACE, digest: sha256(WORKSPACE) }])
  assert.equal(built.evidenceIdentity, p7WorkspaceReferenceEvidenceBindingIdentity(input))
  assert.deepEqual(validateP7WorkspaceReferenceEvidenceBinding(built, input), built)
})

test("P7-R10 identity is deterministic and binds the historical workspace reference and digest", () => {
  const input = fixtureInput()
  const first = buildP7WorkspaceReferenceEvidenceBinding(input)
  const same = buildP7WorkspaceReferenceEvidenceBinding(structuredClone(input))
  ...
  check.evidence[0].ref = "C:\\workspace\\kodac"
  check.evidence[0].digest = sha256(check.evidence[0].ref)
  const second = buildP7WorkspaceReferenceEvidenceBinding(rebuildWithReport(report))
  assert.equal(second.workspaceReference, "C:\\workspace\\kodac")
...
test("P7-R10 rejects P7-R9 predecessor tamper", () => {
  const input = fixtureInput()
  const forged = structuredClone(input.sourceAgentCompletionEvidenceBinding) as MutableRecord
  forged.evidenceIdentity = "f".repeat(64)
  assert.throws(
    () => buildP7WorkspaceReferenceEvidenceBinding({ ...input, sourceAgentCompletionEvidenceBinding: forged as any }),
    /evidenceIdentity|canonical source-derived/,
  )
})

test("P7-R10 requires exact passing workspace.integrity report-check linkage", () => {
  const input = fixtureInput()
  const cases: Array<[string, (check: MutableRecord) => void, RegExp]> = [
    ["category", (check) => { check.category = "custom" }, /workspace.integrity|category|required base check/],
    ["status", (check) => { check.status = "fail" }, /pass|passed|check/],
    ["wrong kind", (check) => { check.evidence = [{ kind: "artifact", ref: WORKSPACE, digest: sha256(WORKSPACE) }] }, /kind|workspace/],
    ["extra evidence", (check) => { check.evidence.push({ kind: "workspace", ref: "/other", digest: sha256("/other") }) }, /exactly one workspace evidence reference|at most 1/],
    ["missing digest", (check) => { check.evidence = [{ kind: "workspace", ref: WORKSPACE }] }, /missing required field|digest/],
    ["malformed digest", (check) => { check.evidence[0].digest = "bad" }, /SHA-256|64 lowercase hexadecimal/],
    ["mismatched digest", (check) => { check.evidence[0].digest = "0".repeat(64) }, /SHA-256 of the exact workspace reference/],
  ]
  for (const [_name, mutate, expected] of cases) {
    const report = reportFrom(input)
    const check = report.checks.find((candidate: MutableRecord) => candidate.id === "workspace.integrity")
    mutate(check)
    assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(rebuildWithReport(report)), expected)
  }
})

test("P7-R10 rejects missing or duplicate workspace.integrity checks", () => {
  const input = fixtureInput()
  const missing = reportFrom(input)
  missing.checks = missing.checks.filter((check: MutableRecord) => check.id !== "workspace.integrity")
  assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(rebuildWithReport(missing)), /workspace.integrity|required base check/)

  const duplicate = reportFrom(input)
  duplicate.checks.push(structuredClone(duplicate.checks.find((check: MutableRecord) => check.id === "workspace.integrity")))
  assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(rebuildWithReport(duplicate)), /duplicate|exactly one|check id/)
})

test("P7-R10 rejects empty, control-bearing, invalid-Unicode, and oversized workspace references", () => {
  const input = fixtureInput()
  const cases: Array<[string, RegExp]> = [
    ["", /empty|must not be empty/],
    ["/workspace/\u0000kodac", /control/],
    ["\ud800", /Unicode scalar/],
    ["x".repeat(1_025), /1024|exceeds/],
  ]
  for (const [ref, expected] of cases) {
    const report = reportFrom(input)
    const item = report.checks.find((check: MutableRecord) => check.id === "workspace.integrity").evidence[0]
    item.ref = ref
    item.digest = sha256(ref)
    assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(rebuildWithReport(report)), expected)
  }
})

test("P7-R10 rejects hostile build and report object graphs", () => {
  const input = fixtureInput()
  assert.throws(
    () => buildP7WorkspaceReferenceEvidenceBinding(new Proxy(input, {}) as P7WorkspaceReferenceEvidenceBindingBuildInput),
    /Proxy/,
  )

  const accessor = { ...input } as MutableRecord
  Object.defineProperty(accessor, "sourceAgentCompletionEvidenceBinding", {
    get: () => input.sourceAgentCompletionEvidenceBinding,
    enumerable: true,
  })
  assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(accessor as any), /enumerable data property/)

  const symbol = { ...input } as MutableRecord
  ;(symbol as any)[Symbol("hidden")] = true
  assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(symbol as any), /symbol fields/)

  const custom = { ...input } as MutableRecord
  Object.setPrototypeOf(custom, { marker: true })
  assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(custom as any), /plain object/)

  const unknown = { ...input, verified: true } as any
  assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(unknown), /unknown field/)

  const report = reportFrom(input)
  const check = report.checks.find((candidate: MutableRecord) => candidate.id === "workspace.integrity")
  check.evidence[0] = new Proxy(check.evidence[0], {})
  assert.throws(() => buildP7WorkspaceReferenceEvidenceBinding(rebuildWithReport(report)), /Proxy/)
})

test("P7-R10 output is detached and deeply immutable, and validator rejects forged semantics", () => {
  const input = fixtureInput()
  const built = buildP7WorkspaceReferenceEvidenceBinding(input)
  const before = structuredClone(built)
  const report = input.sourceAgentCompletionEvidenceBindingInput.sourceCommandSuccessEvidenceBindingInput
    .sourceVerificationReportBindingInput.verificationReport as MutableRecord
  report.checks.find((check: MutableRecord) => check.id === "workspace.integrity").summary = "Mutated after build."
  assert.deepEqual(built, before)
  assert.ok(Object.isFrozen(built))
  assert.ok(Object.isFrozen(built.workspaceCheckEvidence))
  assert.ok(Object.isFrozen(built.workspaceCheckEvidence[0]))

  const forged = structuredClone(built) as MutableRecord
  forged.workspaceReferenceDigest = "f".repeat(64)
  assert.throws(
    () => validateP7WorkspaceReferenceEvidenceBinding(forged, fixtureInput()),
    /canonical source-derived semantics|evidenceIdentity/,
  )
})

test("P7-R10 identity binds the exact workspace check summary", () => {
  const input = fixtureInput()
  const first = buildP7WorkspaceReferenceEvidenceBinding(input)
  const report = reportFrom(input)
  report.checks.find((check: MutableRecord) => check.id === "workspace.integrity").summary =
    "A different but still passing historical workspace reference summary."
  const second = buildP7WorkspaceReferenceEvidenceBinding(rebuildWithReport(report))
  assert.notEqual(second.verificationReportIdentity, first.verificationReportIdentity)
  assert.notEqual(second.workspaceCheckSummary, first.workspaceCheckSummary)
  assert.notEqual(second.evidenceIdentity, first.evidenceIdentity)
})

test("P7-R10 schema mirrors the runtime output boundary", () => {
  assert.equal(schema.$id, "https://kodac.dev/schema/p7-workspace-reference-evidence-binding.schema.json")
  assert.equal(schema.additionalProperties, false)
  assert.equal(schema.properties.version.const, P7_R10_WORKSPACE_REFERENCE_EVIDENCE_BINDING_VERSION)
  assert.equal(schema.properties.state.const, P7_R10_WORKSPACE_REFERENCE_EVIDENCE_BOUND_STATE)
  assert.equal(schema.properties.workspaceCheckEvidence.minItems, 1)
  assert.equal(schema.properties.workspaceCheckEvidence.maxItems, 1)
  assert.equal(schema.properties.workspaceCheckEvidence.items.additionalProperties, false)
  assert.equal(schema.properties.workspaceCheckEvidence.items.properties.kind.const, "workspace")
  assert.equal(schema.properties.workspaceCheckEvidence.items.properties.digest.pattern, "^[0-9a-f]{64}$")
...
test("P7-R10 production source has no prohibited execution surface", () => {
