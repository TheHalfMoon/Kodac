...
import {
  P7_R12_RECEIPT_REPORT_EVIDENCE_BINDING_VERSION,
  P7_R12_RECEIPT_REPORT_EVIDENCE_BOUND_STATE,
  buildP7ReceiptReportEvidenceBinding,
  p7ReceiptReportEvidenceBindingIdentity,
...
const SESSION_ID = "session-p7-r12"
...
function claim(): Record<string, unknown> {
  return {
    claimKey: "p7-r12-source-finding",
    review: {
      reviewRunId: "review-run-p7-r12",
      reviewerId: "provider:test",
      reviewerVersion: "v1",
      policyIdentity: "policy:p7-r12-test",
      canonicalBase: BASE,
      reviewedHead: HEAD,
    },
    path: "src/example.ts",
    range: { startLine: 10, endLine: 12 },
    summary: "A confirmed finding eligible for bounded receipt-report evidence binding.",
...
function sourceProposal(): P7ImmutablePatchProposal {
  const runtime = new ReviewerIntelligenceRuntime({ adjudicatorId: "kodac:p7-r12-test-adjudicator" })
  ...
  return buildP7ImmutablePatchProposal({
    ...
    proposerIdentity: "kodac:p7-r12-test-proposer",
...
function sourceAuthorization(source: P7ImmutablePatchProposal): P7PatchApplicationAuthorization {
  return buildP7PatchApplicationAuthorization({
    ...
    declaration: {
      authorizerIdentity: "kodac:p7-r12-test-authorizer",
...
function verificationReport(plan: P7VerificationPlanInput): MutableRecord {
  ...
  const base = [
    ...
    },
    {
      ...
      category: "policy",
      status: "pass",
      summary: "Every persisted execution receipt was authorized by policy.",
      evidence: RECEIPT_IDS.map((ref) => ({ kind: "receipt", ref })),
    },
  ]
...
function fixtureInput(report?: MutableRecord): P7ReceiptReportEvidenceBindingBuildInput {
...
function reportFrom(input: P7ReceiptReportEvidenceBindingBuildInput): MutableRecord {
  return structuredClone(
    input.sourceGitChangeReportEvidenceBindingInput.sourceWorkspaceReferenceEvidenceBindingInput
      .sourceAgentCompletionEvidenceBindingInput.sourceCommandSuccessEvidenceBindingInput
      .sourceVerificationReportBindingInput.verificationReport,
  ) as MutableRecord
}

function rebuildWithReport(report: MutableRecord): P7ReceiptReportEvidenceBindingBuildInput {
...
const schema = JSON.parse(
  readFileSync(new URL("../../../schema/p7-receipt-report-evidence-binding.schema.json", import.meta.url), "utf8"),
) as UnknownRecord
const sourceText = readFileSync(
  new URL("../src/remediation/p7-receipt-report-evidence-binding.ts", import.meta.url),
  "utf8",
)
...
test("P7-R12 builds and validates one deterministic RECEIPT_REPORT_EVIDENCE_BOUND record", () => {
  ...
  assert.equal(built.version, P7_R12_RECEIPT_REPORT_EVIDENCE_BINDING_VERSION)
  assert.equal(built.state, P7_R12_RECEIPT_REPORT_EVIDENCE_BOUND_STATE)
  assert.equal(built.receiptReportCheckSummary, RECEIPT_SUMMARY)
  assert.equal(built.receiptReportCount, RECEIPT_IDS.length)
  assert.deepEqual(built.receiptReportRefs, [...RECEIPT_IDS].sort())
  assert.deepEqual(
    built.receiptReportEvidence,
    [...RECEIPT_IDS].sort().map((ref) => ({ kind: "receipt", ref })),
  )
  assert.equal(built.evidenceIdentity, p7ReceiptReportEvidenceBindingIdentity(input))
  assert.deepEqual(validateP7ReceiptReportEvidenceBinding(built, input), built)
})

test("P7-R12 canonicalizes receipt-reference ordering without changing identity", () => {
  const input = fixtureInput()
  const first = buildP7ReceiptReportEvidenceBinding(input)
  const report = reportFrom(input)
  receiptCheck(report).evidence.reverse()
  const second = buildP7ReceiptReportEvidenceBinding(rebuildWithReport(report))
  assert.deepEqual(second.receiptReportEvidence, first.receiptReportEvidence)
  assert.deepEqual(second.receiptReportRefs, first.receiptReportRefs)
...
test("P7-R12 identity binds the report count claim and receipt refs", () => {
  const input = fixtureInput()
  const first = buildP7ReceiptReportEvidenceBinding(input)

  const countReport = reportFrom(input)
  receiptCheck(countReport).summary = "4 execution receipt(s) are successful and mutation post-state is attested."
  receiptCheck(countReport).evidence.pop()
  const countChanged = buildP7ReceiptReportEvidenceBinding(rebuildWithReport(countReport))
  assert.equal(countChanged.receiptReportCount, 4)
  ...
  receiptCheck(refReport).evidence[0].ref = "723e4567-e89b-42d3-a456-426614174006"
  const refChanged = buildP7ReceiptReportEvidenceBinding(rebuildWithReport(refReport))
...
test("P7-R12 rejects P7-R11 predecessor tamper", () => {
  const input = fixtureInput()
  const forged = structuredClone(input.sourceGitChangeReportEvidenceBinding) as MutableRecord
  forged.evidenceIdentity = "f".repeat(64)
  assert.throws(
    () => buildP7ReceiptReportEvidenceBinding({ ...input, sourceGitChangeReportEvidenceBinding: forged as any }),
    /evidenceIdentity|canonical source-derived/,
  )
})

test("P7-R12 requires one exact passing evidence.receipts check", () => {
  const input = fixtureInput()

  const missing = reportFrom(input)
  missing.checks = missing.checks.filter((check: MutableRecord) => check.id !== "evidence.receipts")
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(missing)), /evidence.receipts|required base check/)

  const duplicate = reportFrom(input)
  duplicate.checks.push(structuredClone(receiptCheck(duplicate)))
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(duplicate)), /duplicate|exactly one|check id/)

  const category = reportFrom(input)
  receiptCheck(category).category = "custom"
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(category)), /receipts|category|required base check/)

  const status = reportFrom(input)
  receiptCheck(status).status = "fail"
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(status)), /pass|passed|status|check/)
})

test("P7-R12 rejects noncanonical, zero, unsafe, and oversized count summaries", () => {
  const input = fixtureInput()
  const cases: Array<[string, RegExp]> = [
    ["Receipts valid.", /canonical passing summary grammar/],
    ["05 execution receipt(s) are successful and mutation post-state is attested.", /canonical passing summary grammar/],
    ["0 execution receipt(s) are successful and mutation post-state is attested.", /canonical passing summary grammar/],
    ["-1 execution receipt(s) are successful and mutation post-state is attested.", /canonical passing summary grammar/],
    ["257 execution receipt(s) are successful and mutation post-state is attested.", /256|safe integer/],
    [`${"9".repeat(400)} execution receipt(s) are successful and mutation post-state is attested.`, /256|safe integer/],
    ["5 execution receipts are successful and mutation post-state is attested.", /canonical passing summary grammar/],
  ]
  for (const [summary, expected] of cases) {
    const report = reportFrom(input)
    receiptCheck(report).summary = summary
    assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(report)), expected)
  }
})

test("P7-R12 requires receipt count to equal evidence length", () => {
  const input = fixtureInput()

  const missing = reportFrom(input)
  receiptCheck(missing).evidence.pop()
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(missing)), /length.*receipt count/)

  const extra = reportFrom(input)
  receiptCheck(extra).evidence.push({ kind: "receipt", ref: "723e4567-e89b-42d3-a456-426614174006" })
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(extra)), /length.*receipt count/)
})

test("P7-R12 rejects non-receipt, digested, duplicate, and unsafe receipt references", () => {
  const input = fixtureInput()
  const cases: Array<[(check: MutableRecord) => void, RegExp]> = [
    ...
    [(check) => { check.evidence[0].ref = check.evidence[1].ref }, /duplicate|unique/],
  ...
  for (const [mutate, expected] of cases) {
    const report = reportFrom(input)
    mutate(receiptCheck(report))
    assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(report)), expected)
  }
})

test("P7-R12 rejects hostile build and nested report object graphs", () => {
  const input = fixtureInput()
  assert.throws(
    () => buildP7ReceiptReportEvidenceBinding(new Proxy(input, {}) as P7ReceiptReportEvidenceBindingBuildInput),
    /Proxy/,
  )

  const accessor = { ...input } as MutableRecord
  Object.defineProperty(accessor, "sourceGitChangeReportEvidenceBinding", {
    get: () => input.sourceGitChangeReportEvidenceBinding,
    enumerable: true,
  })
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(accessor as any), /enumerable data property/)

  const symbol = { ...input } as MutableRecord
  ;(symbol as any)[Symbol("hidden")] = true
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(symbol as any), /symbol fields/)

  const custom = { ...input } as MutableRecord
  Object.setPrototypeOf(custom, { marker: true })
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(custom as any), /plain object/)

  const unknown = { ...input, verified: true } as any
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(unknown), /unknown field/)

  const proxyReport = reportFrom(input)
  receiptCheck(proxyReport).evidence[0] = new Proxy(receiptCheck(proxyReport).evidence[0], {})
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(proxyReport)), /Proxy/)

  const sparseReport = reportFrom(input)
  delete receiptCheck(sparseReport).evidence[1]
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(sparseReport)), /sparse|array/)

  const aliasReport = reportFrom(input)
  receiptCheck(aliasReport).evidence[1] = receiptCheck(aliasReport).evidence[0]
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(aliasReport)), /alias|duplicate|unique/)

  const cyclicReport = reportFrom(input)
  receiptCheck(cyclicReport).evidence[0].cycle = cyclicReport
  assert.throws(() => buildP7ReceiptReportEvidenceBinding(rebuildWithReport(cyclicReport)), /cycle|unknown field/)
})

test("P7-R12 output is detached and deeply immutable and validator rejects forged semantics", () => {
  const input = fixtureInput()
  const built = buildP7ReceiptReportEvidenceBinding(input)
  const before = structuredClone(built)
  const report = input.sourceGitChangeReportEvidenceBindingInput.sourceWorkspaceReferenceEvidenceBindingInput
    .sourceAgentCompletionEvidenceBindingInput.sourceCommandSuccessEvidenceBindingInput
    .sourceVerificationReportBindingInput.verificationReport as MutableRecord
  receiptCheck(report).summary = "4 execution receipt(s) are successful and mutation post-state is attested."
  receiptCheck(report).evidence.pop()
  assert.deepEqual(built, before)
  assert.ok(Object.isFrozen(built))
  assert.ok(Object.isFrozen(built.receiptReportEvidence))
  assert.ok(Object.isFrozen(built.receiptReportEvidence[0]))
  assert.ok(Object.isFrozen(built.receiptReportRefs))

  const forged = structuredClone(built) as MutableRecord
  forged.receiptReportCount = 4
  assert.throws(
    () => validateP7ReceiptReportEvidenceBinding(forged, fixtureInput()),
    /canonical source-derived semantics|evidenceIdentity/,
  )
})

test("P7-R12 rejects report-level and nested predecessor tamper instead of trusting projected fields", () => {
  const input = fixtureInput()
  const tamperedR6 = structuredClone(
    input.sourceGitChangeReportEvidenceBindingInput.sourceWorkspaceReferenceEvidenceBindingInput
      .sourceAgentCompletionEvidenceBindingInput.sourceCommandSuccessEvidenceBindingInput.sourceVerificationReportBinding,
  ) as MutableRecord
  tamperedR6.verificationReportIdentity = "f".repeat(64)
  const tampered = structuredClone(input) as MutableRecord
  tampered.sourceGitChangeReportEvidenceBindingInput.sourceWorkspaceReferenceEvidenceBindingInput
    .sourceAgentCompletionEvidenceBindingInput.sourceCommandSuccessEvidenceBindingInput.sourceVerificationReportBinding = tamperedR6
  assert.throws(
    () => buildP7ReceiptReportEvidenceBinding(tampered as unknown as P7ReceiptReportEvidenceBindingBuildInput),
...
test("P7-R12 schema mirrors the runtime output boundary and canonical output constraints", () => {
  ...
  assert.equal(schema.properties.version.const, P7_R12_RECEIPT_REPORT_EVIDENCE_BINDING_VERSION)
  assert.equal(schema.properties.state.const, P7_R12_RECEIPT_REPORT_EVIDENCE_BOUND_STATE)
  assert.equal(schema.properties.receiptReportCount.minimum, 1)
  assert.equal(schema.properties.receiptReportCount.maximum, 256)
  assert.equal(schema.properties.receiptReportEvidence.minItems, 1)
  assert.equal(schema.properties.receiptReportEvidence.maxItems, 256)
  assert.equal(schema.properties.receiptReportEvidence.uniqueItems, true)
  assert.equal(schema.properties.receiptReportEvidence.items.additionalProperties, false)
  assert.equal(schema.properties.receiptReportEvidence.items.properties.kind.const, "receipt")
  assert.equal(Object.hasOwn(schema.properties.receiptReportEvidence.items.properties, "digest"), false)
...
test("P7-R12 production source has no prohibited execution or retrospective receipt surface", () => {
